...
pub async fn get_sts_credentials(
    ...
    // 从环境变量获取存储桶名称
    let bucket = std::env::var("COS_BUCKET")
        .map_err(|_| {
    ...
    crate::log_with_storage!(info, "正在请求STS临时凭证，持续时间: {}秒", duration_seconds);
...
pub async fn validate_file_upload(
    ...
    if request.file_size > MAX_FILE_SIZE {
        return Json(FileValidationResponse {
            valid: false,
            message: format!("文件大小超过限制，最大允许{}MB", MAX_FILE_SIZE / 1024 / 1024),
            suggested_key: None,
        });
    }
    ...
    let is_allowed_type = allowed_types.iter().any(|&t| request.content_type.starts_with(t));
    ...
    let suggested_key = format!("media/{}_{}_{}.{}", timestamp, uuid, 
                               sanitize_filename(&request.filename), extension);
    
    crate::log_with_storage!(info, "文件验证通过: {}, 建议键名: {}", request.filename, suggested_key);
